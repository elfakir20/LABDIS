...
if stores_df is not None:
    ...
    if uploaded:
        ...
        if data.empty:
            ...
            # --- FINANCIAL CALCULATION (Stability Patch Included) ---
            results = []
            for i, t in enumerate(trucks_list):
                ...
                # Stability Check: Prevent TypeError if price is missing or invalid
                try:
                    raw_val = p_match.iloc[0]['Tarif'] if not p_match.empty else 0.0
                    base_price = float(raw_val)
