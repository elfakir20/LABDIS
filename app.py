...
st.title("🚚 LABDIS Fleet Manager (Fixed Version)")
...
if stores_df is not None:
    ...
    if uploaded:
        ...
        if data.empty:
            ...
            for (zone, truck_limit), group in data.groupby(['Zone', 'Max_Truck_Allowed']):
                ...
                for _, row in group.iterrows():
                    if cur_load + row['Total_PLT'] <= max_cap:
                        ...
                        if cur_stores: # تأكد أن الشاحنة ليست فارغة
                            all_trucks.append({"Zone": zone, "Type": truck_limit, "Load": cur_load, "Stores": cur_stores, "Cities": cur_cities, "Fleg": cur_fleg, "Sec": cur_sec, "Cap": nominal_cap})
            ...
            v_col, c_col, a_col, p_col = get_col(tariffs_df, ['Véhicule', 'Vehicule', 'Truck']), get_col(tariffs_df, ['Ville', 'City']), get_col(tariffs_df, ['Activité', 'Activite']), get_col(tariffs_df, ['Tarif', 'Price'])
            ...
            for i, t in enumerate(all_trucks):
                ...
                # --- الإصلاح هنا: حماية ضد القوائم الفارغة ---
                main_city = "Unknown"
                if t["Cities"]:
                    main_city = max(set(t["Cities"]), key=t["Cities"].count)
                
                base_p = 0
                if all([v_col, c_col, a_col, p_col]) and main_city != "Unknown":
                    p_match = tariffs_df[(tariffs_df[c_col] == main_city) & (tariffs_df[v_col] == t["Type"]) & (tariffs_df[a_col] == main_act)]
                    if not p_match.empty: base_p = p_match.iloc[0][p_col]

                total_c = base_p + (len(t["Stores"]) - 1) * (STOP_FLEG if main_act == "Fleg" else STOP_SEC)
                results.append({"ID": f"TRK-{i+1:02d}", "Zone": t["Zone"], "Type": t["Type"], "Route": " ➡️ ".join(dict.fromkeys(t["Cities"])), "Stores": " | ".join(t["Stores"]), "PLT": round(t["Load"], 1), "Efficiency_%": round(fill_rate, 1), "Cost (MAD)": total_c})
    ...
    st.error("Check master files on GitHub.")
